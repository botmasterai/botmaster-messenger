...
class MessengerBot extends BaseBot {
  ...
  __checkOptionalCredentials(settings) {
    ...
    if (settings.credentials.pages) {
      for (const pageId of settings.credentials.pages) {
 ...
 */
  ...
  __sendMessage(message) {
    const pageToken = this.credentials.pages ?
      this.credentials.pages[message.sender.id].pageToken :
      this.credentials.pageToken;
    const options = {
      uri: baseMessageURL,
